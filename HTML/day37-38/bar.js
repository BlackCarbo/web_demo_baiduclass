//绘制条形图
function drowHistogram(data, newSelect,isChoose=-1) {
    ...
    for(let m=0;m<num;m++){
        if(newSelect[m]){
            ...
            chartTitle.setAttributeNS(null,"x",100+(T%3)*150);
            chartTitle.setAttributeNS(null,"y",340+Math.floor(T/3)*25);
            ...
            mRect.setAttribute("x", 80+(T%3)*150);
            mRect.setAttribute("y", 330+Math.floor(T/3)*25);
            mRect.setAttribute("width", 10);
            mRect.setAttribute("height", 10);
            mRect.setAttribute( "style", `fill:${rectColor[T]};stroke-width:1;stroke:rgb(0,0,0)`);
    ...
    for(let w=0;w<num;w++){
        if(newSelect[w]){
            for(let i=0;i<12;i++){
                ...
                rect.setAttribute("x", 30+(i+1)*rectSpace+i*rectWidth+N*rectWidth/M);
                rect.setAttribute("y", chartHeight-20-data[w].sale[i]/ratio);
                rect.setAttribute("width", rectWidth/M);
                rect.setAttribute("height", data[w].sale[i]/ratio);
                rect.setAttribute( "style", `fill:${rectColor[N]};stroke-width:1;stroke:rgb(0,0,0)`);
